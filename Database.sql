...
BEGIN
	...
	;WITH BillShow AS (SELECT b.id, t.name AS [Bàn], b.totalPrice AS [Tổng hóa đơn], b.DateCheckIn AS [Ngày vào], b.DateCheckOut AS [Ngày ra], b.discount AS [Giảm giá]
	FROM dbo.Bill AS b, dbo.TableFood AS t
	WHERE b.status=1 AND t.id=b.idTable AND b.DateCheckIn>=@checkIn AND b.DateCheckOut<=@checkOut)
	
	SELECT TOP (@selectRows) *FROM BillShow WHERE id NOT IN (SELECT TOP (@selectRows) id FROM BillShow)
END
GO
